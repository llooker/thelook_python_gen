...
def main():
    ...
    for oi in order_items:
        for key in order_item_remove_keys:
            del oi[key]
    ...
    for name, table_dat in list(zip(table_name, table_dat)):
        ...
        logging.info(f"uploading {name} to GCS")
        upload_to_bucket(bucket_name="thelook-faker", file_name=name, data=str(csv_data))
...
def generate_products():
    ...
    return product_gender_dict, product_by_id_dict
...
def upload_to_bucket(bucket_name, file_name, data):
    storage_client = storage.Client.from_service_account_json('my-personal-instance-5e4d7dfb341a.json')

    buckets = list(storage_client.list_buckets())
    print(buckets)
    bucket = storage_client.get_bucket(bucket_name)
    blob = bucket.blob("{}.csv".format(file_name))
...
class Users(DataUtil):
    ...
    def __post_init__(self):
        ...
        address = Address(get_address(country='United States'))
        ...
        self.email = self.first_name + self.last_name + "@" +  fake.free_email_domain()
        year = datetime.now().year
        month = datetime.now().month
        day = datetime.now().day
        
        choice = random.choices([0,1], weights=[.9,.1])[0]
        if choice == 0:
...
class Order_Item(DataUtil):
    ...
    def __post_init__(self, order=None):
        ...
        if order.num_of_item == 1: #if only 1 item in order go through flow
            ...
            # purchase_time = None #time of previous purchase
            for i in range(order.num_of_item): 
                for j in ["department", "product", "cart"]:
                    ...
                    previous_created_at = self.created_at
                    self.created_at = previous_created_at + timedelta(seconds=randrange(180))
                    # self.created_at = purchase_time + timedelta(minuts=randrange)
...
class Inventory_Item:
    ...
    order_item:InitVar[Any] = None
    def __post_init__(self, order_item=None):
        self.inventory_item_id = order_item.inventory_item_id
        self.product_id = order_item.product_id
        if order_item.is_sold is True:
        # if order_item.created_at is not None:
        ...
        # if order_item.created_at is None:
    ...
    def __str__(self):
        return f'{self.inventory_item_id}, {self.product_id}, {self.created_at}, {self.cost}, {self.product_category}, {self.product_name}, {self.product_brand}, {self.product_retail_price}, {self.product_department}, {self.product_sku}, {self.product_distribution_center_id}'
...
class Ghost_Events(DataUtil):
    ...
    def __post_init__(self):
        ...
        #abandoned cart
        for i in ["department", "product", "cart"]:
            #set ghost events ID to max of original
            event_id = len(events)
            self.id = event_id + 1
            event_id = self.id
